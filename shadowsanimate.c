...
void colorPixel(Color* h, Ray p, Sphere sph, double t)  //costheta = n * s
{
   Vector q;
   ...
   //
   h->r = abs((int)(0.5*sph.h.r*(1+cosTheta)));
   h->g = abs((int)(0.5*sph.h.g*(1+cosTheta)));
   h->b = abs((int)(0.5*sph.h.b*(1+cosTheta)));
   /*
...
void itrItem(int k) //k value for determining frame adjustment
{
   ...
   for( y = 0 ; y < H ; y++ )
   {
      //#pragma omp parallel for private(x,h,p.end)
      for( x = 0 ; x < W ; x++)
      {
         //Correction
         px = ((x * 1.0 +.05) *( 4.0/3 ))/W -(1.0/6) ;
         py = 1 -((y * 1.0 +.05) *( 1.0/H )) ;
         pz = 0.0;
         //
         ...
         unitVector(&p.end);
         //
         minT = 100000.0; //BIGNUMBER
         //
         for(i = 0; i < codeSpheres + fileSpheres; i++)
         {
            t = hitSphere(p, items[i]);
            //
            if(t > 0 && t < minT)  //If it actually hits
            {
               colorPixel(&h, p, items[i], t); //Get the pixel color w/ shadow
               //
               rgb[y][x][0] = h.r;
               rgb[y][x][1] = h.g;
               rgb[y][x][2] = h.b;
